...
class CommunityStreamHandler2(PLHandler):
    ...
    def create(self, request):
        ...
        items, sources = self.addItemDetails(items, loginOrg, includeSourceDetails = includeSourceDetails, remoteAddr = self.request.META.get('REMOTE_ADDR'))
        for item in items:
            for key in timeKeys:
                if key in item:
    ...
    #--------------------------------------------------------------------------- addItemDetails
    def addItemDetails(self, items, loginOrg, includeSourceDetails=False, remoteAddr=None):
