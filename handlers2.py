...
from pt.pttimezone.pttimezone import changeTZ, getSystemTimezone, now_in_timezone, today_in_timezone
...
class CommunityStreamHandler2(PLHandler):
    ...
    def create(self, request):
        ...
        ptlog.syslog("=========------------ loginOrg :{}".format(loginOrg))
        ptlog.syslog("=========------------ loginOrgTZ :{}".format(loginOrgTZ))
        ...
        ptlog.syslog("--------------------- reponse :{}".format(ret))
        return ret
    ...
    def getOrgTimezone(self, orgID):
        ...
        ptlog.syslog(">>>>>>>>>>>>>>>>>>>>>> timezoneMap[timezoneName].get_pytz() :{}".format(timezoneMap[timezoneName].get_pytz()))
    ...
    def addItemDetails(self, items, includeSourceDetails=False, remoteAddr=None):
        ...
        for group in ('messages', 'alerts', 'calendarEvents', 'feedEntries', 'media', 'flyers', 'marketplace', 'cafeteriaMenuDays'):
            ...
            if group == 'messages':
                ...
                ptlog.syslog("================================= Inside calendarEvents =======================")
                ...
                if calendarEventIDs:
                    ...
                    for eventDict in groupItems:
                        ptlog.syslog("================ eventDict :{}".format(eventDict))
                        event = events[eventDict['calendarEventID']]
                        ptlog.syslog("================....... event :{}".format(event))
                        eventDict['summary'] = event['summary']
                        #ptlog.syslog(",,,,,,,,,,,,,,,,,,,,,,,,,,,, startDateTime :{}".format(eventDict['startDateTime'].strftime("%Y-%m-%d, %H:%M:%S")))


                        if eventDict.get('startDateTime'):
                            ptlog.syslog("================== Before : {}".format(eventDict['startDateTime']))
                            ...
                            ptlog.syslog("================== After : {}".format(eventDict['startDateTime']))
                ...
                if not tiebreaker or instance['eventInstanceID'] not in tiebreaker:
                    for key in ['endDateTime', 'startDateTime', 'dtstart', 'until']:
                        if key in instance:
                            ...
                            #instance[key] = getDatetime(instance[key], calTZ)
